...
<template>
  <div class = "relative w-screen h-screen overflow-hidden">
    <img :src = "selectedList.background_image" alt = "background mantenimiento" class = "w-full h-full absolute"/>
    <div :class = "`absolute mt-8 sm:mt-2 h-[60%] sm:h-screen flex flex-col items-center justify-evenly sm:top-0 top-1/3 ${
      ...
      <div class = "flex justify-around w-1/2">
        <button class = "bg-[#FFD028] text-white font-bold py-2 px-6 rounded-full" @click = "changeList('VS')">
          VS
        </button>
        <button class = "bg-[#D59C00] text-white font-bold py-2 px-6 rounded-full" @click = "changeList('Quota')">
          Quota
        </button>
        <button class = "bg-[#ABC90B] text-white font-bold py-2 px-6 rounded-full" @click = "changeList('VS - Quota')">
          VS - Quota
        </button>
      </div>
    </div>
    <div :class = "`absolute flex flex-col items-center mt-6 gap-2 sm:gap-5 sm:justify-center top-0 sm:h-screen  h-1/3  sm:top-0 ${
      ...
      <h1 :style = "{ color: selectedList.primaryColor }" class = "w-[80%] text-center font-extrabold text-7xl">
        {{ selectedList.christmas_msg }} {{ selectedList.name }} </h1>
    </div>
  </div>
</template>
